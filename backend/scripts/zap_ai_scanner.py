...
class ZAPScanner:
    def __init__(self, target_url, zap_api="http://localhost:8080"):
        ...
        # State tracking for cached values (v4.0)
    ...
    def api_call(self, endpoint, params=None, timeout=60, retries=3):
        """Make ZAP API call with error handling and retries"""
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    ...
                    self.log(f"API timeout on {endpoint}, retry {attempt + 1}/{retries} in {wait_time}s", "WARNING")
                    time.sleep(wait_time)
                else:
                    self.log(f"API call timed out after {retries} attempts: {endpoint}", "ERROR")
                    return None
    ...
    def configure_file_exclusions(self):
        """Configure exclusions for large file types (v4.0)"""
        self.log("Configuring file type exclusions...")
        
        patterns = [
            # Videos (prevent infinite loops)
            r".*\.webm.*", r".*\.mp4.*", r".*\.mov.*", r".*\.avi.*",
            r".*\.mkv.*", r".*\.flv.*", r".*\.wmv.*",
        ...
        for pattern in patterns:
            result = self.api_call("core/action/excludeFromProxy", {"regex": pattern})
            if result:
                excluded_count += 1
        
        self.log(f"Configured {excluded_count}/{len(patterns)} exclusion patterns")
    ...
    def run_active_scan(self):
        ...
        # Monitor progress with error tracking and caching (v4.0)
        ...
        while True:
            ...
            if not status_result:
                ...
                if consecutive_failures >= max_consecutive_failures:
                    self.log(f"Too many consecutive API failures ({max_consecutive_failures}), stopping", "ERROR")
                    break
                self.log(f"Status check failed, will retry (failure {consecutive_failures}/{max_consecutive_failures})", "WARNING")
                time.sleep(30)
                continue
            ...
            if progress == last_progress:
                stuck_count += 1
                if stuck_count > 20:
                    self.log(f"Scan stuck at {progress}% for 5+ minutes", "WARNING")
                    stuck_count = 0
            else:
                stuck_count = 0
            ...
            # Get alert count with caching
            alerts = self.api_call("core/view/numberOfAlerts", timeout=45)
            if alerts and 'numberOfAlerts' in alerts:
                try:
                    alert_count = int(alerts.get('numberOfAlerts', 0))
                ...
                if self.api_timeout_count <= 3:
                    self.log(f"Using cached alert count: {alert_count} (API busy)", "WARNING")
    ...
    def run_full_scan(self, quick_mode=False):
        ...
        # Configure file exclusions BEFORE scanning (v4.0)
        self.configure_file_exclusions()
