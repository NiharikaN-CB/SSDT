...
async function generatePdfReport(scanResult) {
    ...
    // Step 3: Translate AI analysis to Japanese
    console.log('🌐 Step 3/3: Translating AI analysis to Japanese...');
    let aiAnalysisJa = null;
    if (aiAnalysisEn) {
        try {
            aiAnalysisJa = await translateAiAnalysisToJapanese(aiAnalysisEn);
        } catch (error) {
            console.error('⚠️ Failed to translate AI analysis:', error.message);
        }
    }
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            renderReport(doc, scanData, aiAnalysisEn, 'en');
            ...
            renderReport(doc, scanData, aiAnalysisJa, 'ja');
 ...
 */
function renderReport(doc, scanData, aiAnalysis, lang) {
    ...
    switch (block.type) {
        case 'paragraph':
            doc.font('NotoSans')
                .fontSize(10)
                .fillColor(COLORS.text)
                .text(block.text, { width: 495, align: 'left', lineGap: 2 });
            doc.moveDown(0.2);
            break;
